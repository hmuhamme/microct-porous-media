...
# This module was migrated from the original research script: rev_xy.py
...
VOXEL_SIZE = np.array([46, 46, 46])  # µm
...
def create_subvolumes(image, fixed_threshold=55):
    ...
    for n in division_steps:
        ...
        for i in range(n):
            for j in range(n):
                ...
                sub_binary = binary_image[:, y_start:y_end, x_start:x_end]
                if sub_binary.shape[1] == 0 or sub_binary.shape[2] == 0:
                    continue
                pos_key = f"subvol_n{n}_{sub_volume_idx:03d}_at_y{y_start}_x{x_start}"
...
def compute_pore_volume_metrics(subvolumes, positions, voxel_size, output_dir):
    ...
    for n in unique_sizes:
        ...
        for pos in [p for p in positions if f"subvol_n{n}_" in p]:
            ...
            size_um = min(dims[1], dims[2]) * voxel_size  # Height or width in µm
    ...
    pd.DataFrame(all_subvolume_data).to_csv(os.path.join(output_dir, "RHP_1_fullxy_raw_1.csv"), index=False)
    pd.DataFrame(cv_data).to_csv(os.path.join(output_dir, "coefficient_of_variation_RHP_1_cv.csv"), index=False)
    ...
    ax1.errorbar(mean_sizes, phi_vis_means, yerr=phi_vis_stds, fmt='b-o', ecolor='black', capsize=5, capthick=2, label='Mean Visible Porosity of Peat')
    ax1.axhline(y=overall_phi_vis_mean, color='green', linestyle='--', label=f'Overall Mean φ_vis = {overall_phi_vis_mean:.4f}')
    ax1.set_xlabel('Subvolume Size-xy (µm)')
    ...
    ax2.set_ylim(0.0, 0.2)  # Adjust CV axis for better scaling
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "RHP_1_xydivition.tiff"), dpi=400)
    plt.show()
...
def run_rev_xy(image_path, output_dir, voxel_size=VOXEL_SIZE[0], fixed_threshold=55):
    """Run the full 2D REV workflow from image path to output files."""
